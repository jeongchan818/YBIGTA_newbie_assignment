...
def resolve(host: str) -> tuple[list[str], Optional[str]]:
    ...
    try:
        ...
        ips = [ip[4][0] for ip in infos] # TODO: [이곳에 IP 리스트 생성 코드를 작성하세요]
...
def pick_ip(ips: list[str], prefer: str = "any") -> Optional[str]:
    ...
    Args:
        ips (list[str]): IP 주소 리스트
        prefer (str): 선호하는 IP 버전 ("ipv4", "ipv6", "any")
   
     Returns:
        Optional[str]: 선택된 IP 주소 또는 None (리스트가 비어있을 경우)
    ...
    if prefer == "ipv4":
        for ip in ips:
            if ':' not in ip:  # IPv4 주소는 ':'가 없음
                return ip
    elif prefer == "ipv6":
        for ip in ips:
            if ':' in ip:  # IPv6 주소는 ':'가 있음
                return ip
