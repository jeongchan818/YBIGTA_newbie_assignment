...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        h1 = X @ W1 + b1 # 입력값에 가중치를 곱하고 편향을 더한다
        z1 = np.maximum(0,h1) # 연산결과에 Relu 적용

        h1 = X @ W1 + b1
        z1 = relu(h1)                 # 1층: ReLU

        h2 = z1 @ W2 + b2
        z2 = leakyrelu(h2)            # 2층: LeakyReLU 

        h3 = z2 @ W3 + b3
        z3 = tanh(h3 + h1)            # 3층: H3 + H1 후 tanh 적용

        h4 = z3 @ W4 + b4
        y_hat = 1 / (1 + np.exp(-h4)) # 4층: 시그모이드 
        y_hat = y_hat.reshape(-1) # reshape to (N,)
        #################
    ...
    def back_propagation(self, cache, X, y, L2_norm=0.0):
       ...
       # tanh 미분: (1 - z^2)
        dtanh = dh4 @ W4.T
        dh3_total = dtanh * (1 - z3 ** 2)
        
        # H3 + H1 덧셈 노드이므로 기울기가 H3와 H1 양쪽으로 분배
        dh3 = dh3_total
        ...
        # Layer 2: LeakyReLU
        dh2 = dz2 * leakyrelu_grad(h2)     # LeakyReLU 미분
        db2 = np.sum(dh2, axis=0, keepdims=True)
        dW2 = z1.T @ dh2 + 2 * L2_norm * W2
        dz1 = dh2 @ W2.T

        # Layer 1
        # 2층에서 역전파된 기울기(dz1 * relu_grad)에, 3층(H3+H1)에서 넘어온 기울기(dh3_total)를 더해줌
        dh1 = dz1 * relu_grad(h1) + dh3_total
    ...
    def predict(self, X):
        ...
        y_hat, _ = self.forward_propagation(X) # y_hat만 가져오고 cache는 필요없으니 무시
