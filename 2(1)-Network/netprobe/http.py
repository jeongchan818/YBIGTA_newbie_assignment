...
def parse_status_and_preview(raw: bytes, max_preview: int = 200) -> tuple[Optional[int], str, Optional[str]]:
    ...
    Args:
        raw (bytes): 서버로부터 받은 Raw 응답 데이터
        max_preview (int): 바디 영역에서 추출할 최대 미리보기 길이
    Returns:
        tuple: (status_code, preview, error)
            - status_code (int or None): HTTP 상태 코드 (예: 200, 404 등)
            - preview (str): 바디 영역에서 추출한 미리보기 문자열
            - error (str or None): 파싱 중 발생한 에러 메시지
    ...
    preview = ""
    error = None
    # 1. 헤더와 바디의 경계선(빈 줄) 찾기
    separator = b"\r\n\r\n"
    sep_index = raw.find(separator)

    if sep_index == -1:
        error = "Invalid HTTP response: Cannot find header/body separator (\\r\\n\\r\\n)"
        return status_code, preview, error

    try:
        # 2. 헤더와 바디 분리
        header_bytes = raw[:sep_index]
        body_bytes = raw[sep_index + len(separator):]

        # 3. 헤더 디코딩 및 상태 코드 추출
        header_str = header_bytes.decode("utf-8", errors="ignore")
        header_lines = header_str.split("\r\n")
        
        status_line = header_lines[0]         # 예: "HTTP/1.1 200 OK"
        parts = status_line.split(" ")        # 예: ["HTTP/1.1", "200", "OK"]
        ...
        # 4. 바디 영역 디코딩 및 미리보기 추출
        # errors="replace"를 사용해 이미지나 압축 파일 등 텍스트가 아닌 깨진 데이터가 와도 에러가 나지 않게 처리
        body_str = body_bytes.decode("utf-8", errors="replace")
        preview = body_str[:max_preview]
